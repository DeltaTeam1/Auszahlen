...
5. Fuehre `configureAllowedOrigins` aus. Als Argument muss die lokale und die spaetere GitHub-Pages-Origin angegeben werden, beispielsweise:

```javascript
configureAllowedOrigins('http://localhost:5173,https://DEIN-GITHUB-NAME.github.io')
```

Die Origin enthaelt keinen Repository-Pfad. Wenn die GitHub-URL vorliegt, ersetze `DEIN-GITHUB-NAME` durch ihren echten Hostnamen und fuehre diese Funktion erneut aus.
...
2. Lege dieses Verzeichnis als GitHub-Repository an und pushe auf den Branch `main`.
3. Oeffne in GitHub `Settings` > `Pages` und waehle bei `Build and deployment` die Option `GitHub Actions`.
4. Der Workflow [deploy-pages.yml](.github/workflows/deploy-pages.yml) baut den Unterordner `payout-system` und veroeffentlicht ihn. Nach dem ersten erfolgreichen Lauf wird die Pages-URL in der Action angezeigt.
